...
SUGAR_WEIGHTS = [0.4, 0.2, 0.15, 0.15, 0.1]
...
def generate_exercise_data():
    """Generate 1-3 exercise types per day with weighted probabilities"""
    # Determine number of exercise types for the day
    num_exercises = np.random.choice([1, 2, 3], p=NUM_EXERCISE_TYPES_PROBS)
    
    # Select exercise types without replacement, using weighted probabilities
    selected_exercises = []
    available_exercises = EXERCISE_TYPES.copy()
    available_weights = EXERCISE_TYPE_WEIGHTS.copy()
    
    for i in range(num_exercises):
        # Normalize weights for remaining exercises
        normalized_weights = np.array(available_weights) / sum(available_weights)
        
        # Select an exercise type
        selected_idx = np.random.choice(len(available_exercises), p=normalized_weights)
        selected_exercise = available_exercises[selected_idx]
        selected_exercises.append(selected_exercise)
        
        # Remove selected exercise from available options
        available_exercises.pop(selected_idx)
        available_weights.pop(selected_idx)
    
    # Generate exercise data for up to 3 slots
    exercise_data = {
        "Exercise_Type_1": selected_exercises[0] if len(selected_exercises) > 0 else "",
        "Exercise_Minutes_1": random.randint(10, 120) if len(selected_exercises) > 0 else 0,
        "Exercise_Type_2": selected_exercises[1] if len(selected_exercises) > 1 else "",
        "Exercise_Minutes_2": random.randint(10, 120) if len(selected_exercises) > 1 else 0,
        "Exercise_Type_3": selected_exercises[2] if len(selected_exercises) > 2 else "",
        "Exercise_Minutes_3": random.randint(10, 120) if len(selected_exercises) > 2 else 0,
    }
...
# ---------------------------------------------
EXERCISE_TYPES = ["walking", "swimming", "running", "resistance/weights", "biking", "balance/stretching", "other"]
# Exercise type probabilities (Walking has higher probability)
EXERCISE_TYPE_WEIGHTS = [0.4, 0.15, 0.15, 0.05, 0.1, 0.1, 0.05]  # walking(0.4), swimming(0.15), running(0.15), resistance/weights(0.05), biking(0.1), balance/stretching(0.1), other(0.05)
# Number of exercise types per day probabilities
NUM_EXERCISE_TYPES_PROBS = [0.6, 0.3, 0.1]  # 1, 2, 3 types respectively
...
def assign_conditions():
    ...
    - 73% of adults 65+ have at least 1 of 10 common chronic conditions
    - 37% of seniors report having at least two chronic diseases
    
    Conditions with relevant data mappings:
    - Hypertension (65.7%): BP readings, medications
    - Ischemic Heart Disease (27%): BP readings, exercise data
    - Diabetes (26.8%): glucose levels, medications, meal contents
    - Osteoarthritis (25.1%): pain location/level, exercise type
    - Mood/Anxiety Disorders (10.5%): mood tracking, medications, sleep quality
    - Cancer (25.1%): no direct data mapping but included for realism
    ...
    if random.random() < 0.27:   # 27% prevalence
    ...
    if random.random() < 0.105:  # 10.5% prevalence
        conditions.append("Mood or Anxiety Disorders")
    
    # Ensure realistic comorbidity patterns
    # 37% of seniors should have at least 2 conditions
    if len(conditions) < 2 and random.random() < 0.37:
        # Add a second condition if we only have one
        additional_conditions = ["Hypertension", "Diabetes", "Osteoarthritis", "Ischemic Heart Disease"]
    ...
    # Ensure at least one condition (27% have no conditions, but we want all patients to have data)
    if not conditions:
        conditions.append(random.choice(["Hypertension", "Diabetes", "Osteoarthritis"]))
...
def assign_medications_by_conditions(conditions, all_medications):
    ...
    Handles all conditions from the Canadian Community Health Survey:
    - Hypertension, Ischemic Heart Disease: heart health medications
    - Diabetes: diabetes medications
    - Osteoarthritis: pain management medications
    - Mood/Anxiety Disorders: mental health medications
    ...
    for med, med_type, category, dosage in all_medications:
        ...
        elif "Chronic pain and musculoskeletal" in category and "Osteoarthritis" in cond_set:
            should_assign = True
        
        # Mental health conditions
        elif "Mental health" in category and "Mood or Anxiety Disorders" in cond_set:
            should_assign = True
...
def generate_measurement_timestamps_with_spacing(num_readings, min_hours_between=1):
    """Generate realistic timestamps for health measurements with minimum spacing between readings."""
    if num_readings == 0:
        return ["", "", "", ""]
    ...
    # Morning peak: 7-10am (weight: 0.4)
    # Daytime: 10am-10pm (weight: 0.55)
    # Overnight: 10pm-7am (weight: 0.05)
    ...
    for i in range(num_readings):
        ...
        while attempts < max_attempts:
            ...
            if rand < 0.4:  # Morning peak (7-10am)
                hour = random.randint(7, 9)
                minute = random.randint(0, 59)
            elif rand < 0.95:  # Daytime (10am-10pm)
                # Exclude morning hours already covered
                daytime_hours = list(range(10, 22))  # 10am to 9pm
                hour = random.choice(daytime_hours)
                minute = random.randint(0, 59)
            else:  # Overnight (10pm-7am)
                overnight_hours = [22, 23, 0, 1, 2, 3, 4, 5, 6]
                hour = random.choice(overnight_hours)
                minute = random.randint(0, 59)
...
def generate_blood_pressure_readings(has_condition):
    ...
    # Determine number of readings (1-4) with probabilities
    num_readings = np.random.choice([1, 2, 3, 4], p=[0.3, 0.4, 0.2, 0.1])
    ...
    for i in range(4):
        if i < num_readings:
            # Add small probability for low blood pressure readings
            if random.random() < 0.05:  # 5% chance for low BP
                systolic = int(random.uniform(70, 89))  # Low systolic range
                diastolic = int(random.uniform(45, 59))  # Low diastolic range
            else:
...
def generate_glucose_readings(has_diabetes):
    ...
    # Determine number of readings (1-4) with probabilities
    num_readings = np.random.choice([1, 2, 3, 4], p=[0.25, 0.35, 0.25, 0.15])
    ...
    for i in range(4):
        if i < num_readings:
            ...
            if measurement_type == "Pre meal":
                glucose = round(random.uniform(3.0, 9.0), 1)
            else:
                glucose = round(random.uniform(4.0, 12.0), 1)
...
def generate_patient_data(patient_id, conditions, age, gender, patient_name, patient_pain_location, assigned_medications):
    ...
    - Mood/Anxiety Disorders: mood tracking, medications, sleep quality
    - Cancer, COPD, Asthma: general health monitoring (no specific data mappings)
    """
    data = []
    cond_set = set(conditions)
    ...
    has_mood_disorder = "Mood or Anxiety Disorders" in cond_set
    
    for date in date_range:
        # Basic data
        mood = np.random.choice(MOOD_VALUES_WITH_EMPTY, p=MOOD_PROBABILITIES)
        weather = random.choice(WEATHER_VALUES)
        sleep_quality_code = np.random.choice([0, 1, 2, 3], p=[0.1, 0.2, 0.4, 0.3])
